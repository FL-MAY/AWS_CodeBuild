...
def exe_path_result_cb(userdata, status, result):
    ...
    if result.outcome == ExePathResult.SUCCESS:
        ...
        rospy.loginfo('Execution of DWAPlannerROS terminated with non-success status code %s:\n%s', str(result.outcome), result.message)
        return 'failure'
...
class ReplanningStateMachine(smach.StateMachine):
    ...
    def __init__(self):
        ...
        #Note by Tony: Hard code.
        self._max_vel_lin = 0.45 #rospy.get_param('/move_base_flex/eband/max_vel_lin')
        self._max_vel_th = 1.0 #rospy.get_param('/move_base_flex/eband/max_vel_th')
    ...
    def get_path_goal_cb(self, userdata, goal):
        ...
        # where the robot may be, if the SBPLLatticePlanner returns. It's calculated by the amount the DWAPlannerROS
